...
跨设备 / 跨发行版共享的 Claude Code 用户级配置（CLAUDE.md / rules / memory / skills / commands）。
...
# 本地的 daemon.json / settings.json / cache/ 等已在 .gitignore 中，不会被覆盖
```
...
**A 设备改了 memory / rules → push**：
```bash
cd ~/.tclaude
git status
git add CLAUDE.md rules/ memory/ skills/ commands/ agents/
git commit -m "memory: <one line>"
git push
```
...
- `.claude.json*` — 会话状态
- `settings.json` — 各发行版 schema 不同，各自维护
